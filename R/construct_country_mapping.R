...
#'
#' @export
construct_country_mapping <- function (x, alpha_code = "iso3c") {
  uv <- unique(x[!is.na(x)])
  scheme <- infer_countrycode_scheme(uv)
  freq_table <- table(x)
  country_mapping <- data.frame(
    Country = countrycode::countrycode(sourcevar = uv, origin = scheme, destination = "country.name", warn = FALSE),
    Code = 0:(length(uv)-1),
    Alpha_Code = countrycode::countrycode(sourcevar = uv, origin = scheme, destination = alpha_code, warn = FALSE),
    Source_Var = uv,
